package com.harvey.w.dubbo.codec;
...
public class DubboCodecExt extends ExchangeCodec implements Codec2 {

	private static final Logger log = LoggerFactory.getLogger(DubboCodecExt.class);
	
	private static final DubboCodec DUBBOCODEC = new DubboCodec();
	...
	protected void encodeResponseData(Channel channel, ObjectOutput out, Object data) throws IOException {
		...
		if (th == null) {
			...
			if (ret == null) {
				out.writeByte(DubboCodec.RESPONSE_NULL_VALUE);
			} else {
				out.writeByte(DubboCodec.RESPONSE_VALUE);
				out.writeObject(ret);
			}
			out.writeObject(result.getAttachments());
		} else {
			out.writeByte(DubboCodec.RESPONSE_WITH_EXCEPTION);
			out.writeObject(th);
		}
	}
}
